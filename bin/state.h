...
struct CartState {
    ...
    double xori0_static[3*TOTMAXATOM];  //for sidechain-switching, will not be switched and compared later
    ...
    int natco[TOTMAXRES];
    //sidechain switching
    static const bool mcdebug=false;
    std::vector<int> resWcopies; //"list" of residues with copies, for sidechain switching
    int lastResidueSwitched;
    int lastCopySwitched;
...
struct MiniState {
    ...
    double mctemp; //Monte Carlo temperature (in KT) // in what? meV??
    ...
    double mcensprob; //Monte Carlo probability of switching ensemble copies -> superseded by mcprobs now
