...
c      Local variables: matrices
         real*8 rotmatr, rotmatl, rotmatd, rotmatd2, rotmatrinv, 
     1      rotmatlinv
...
c apply ensemble/normal mode deformations to receptor and ligand
     ...
     1    morphr, cmorphr, ptr_cmorphdr)
         call deform(ensr, ensdr, cmorphr, cmorphdr, dligr, nhm, nihm, 
     1    idr,ieins, eig, index_eig, index_val, xb, x, xori, xori0, 1)
         call cartstate_get_ensd(cartstatehandle, idl, ensl, ptr_ensdl,
     1    morphl, cmorphl, ptr_cmorphdl)
         call deform(ensl, ensdl, cmorphl, cmorphdl, dligl, nhm, nihm,
     1    idl,ieins, eig, index_eig, index_val, xb, x, xori, xori0, 1)
...
c select deformed coordinates: select non-pivotized coordinates for receptor
     ...
     1      ptr_rbc, ptr_rc, ptr_ac, ptr_emin, ptr_rmin2, ptr_ipon, 
     2      potshape, cdie, epsilon, swi_on, swi_off)
...
c      therefore, we must rotate the pm2 matrix
               ...
               do j = 1, 3
                  do l = 1, 3
                     mat = rotmatrinv
                     pm2(1, j, l) = mat(1) * pm20(1, j, l) + mat(2) * 
     1                  pm20(2, j, l) + mat(3) * pm20(3, j, l)
                     pm2(2, j, l) = mat(4) * pm20(1, j, l) + mat(5) * 
     1                  pm20(2, j, l) + mat(6) * pm20(3, j, l)
                     pm2(3, j, l) = mat(7) * pm20(1, j, l) + mat(8) * 
     1                  pm20(2, j, l) + mat(9) * pm20(3, j, l)
                  enddo
               enddo
            endif
...
c       write(ERROR_UNIT,*), xl(3*132+1:3*132+3), xr(3*1704+1:3*1704+3)
     ...
     1          enon,  epote, cartstatehandle) 
            ...
            call nonbon8(iab, xl, xr, fl, fr, wel, wer,chair,chail,
     1     ac,rc,emin,rmin2, iacir, iacil, nonr, nonl, ipon, nonp,
     2     potshape, cdie, swi_on, swi_off, enon, epote,natomr,natoml)
            endif
         endif

         energies(1) = enon
         energies(2) = epote !coulomb-energy
...
c      multiply the torque matrix with the inverse axsym force rotation matrix
            ...
            do j = 1, 3
               do l = 1, 3
                  pm2a(1, j, l) = mat(1) * pm2(1, j, l) + mat(2) * 
     1              pm2(2, j, l) + mat(3) * pm2(3, j, l)
                  pm2a(2, j, l) = mat(4) * pm2(1, j, l) + mat(5) * 
     1              pm2(2, j, l) + mat(6) * pm2(3, j, l)
                  pm2a(3, j, l) = mat(7) * pm2(1, j, l) + mat(8) * 
     1              pm2(2, j, l) + mat(9) * pm2(3, j, l)
               enddo
            enddo
